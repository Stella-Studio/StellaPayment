...
class DataBase:
    ...
    def _cache_valid(self, version: int) -> bool:
        return (
            ...
            self._cache_version == version and
            (time.time() - self._cache_ts) < self._cache_ttl
        )

    async def _bump_version(self):
        await self.meta.update_one(
            {"_id": "payments_cache"},
            {"$inc": {"version": 1}, "$currentDate": {"updatedAt": True}},
            upsert=True
        )
    ...
    async def save(self, user_id: int, amount: int):
        ...
        if self._cache is not None:
            ...
            if row:
                ...
                self._cache.append({"_id": user_id, "total_amount": amount})
    ...
    async def list_customers(self, use_cache: bool = True):
        ...
        async with self._lock:
            ...
            if use_cache and self._cache_valid(meta_ver):
                return self._cache
            cursor = self.payments.aggregate([
